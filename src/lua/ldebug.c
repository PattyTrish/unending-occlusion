...
StkId aux_stackedfunction (lua_State *L, int level, StkId top) {
  int i;
...
int pushpc (int *stack, int pc, int top, int n) {
  while (n--)
    stack[top++] = pc-1;
  return top;
}
...
Instruction luaG_symbexec (const Proto *pt, int lastpc, int stackpos) {
...
const char *getobjname (lua_State *L, StkId obj, const char **name) {
