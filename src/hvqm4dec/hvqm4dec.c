...
/* Pikmin's fakematch helpers (its types.h), kept while the file is Pikmin's */
#define REPEAT1(x) x
...
/* HVQM4 1.3 reads big-endian fields a byte at a time */
#define read16(buf, offset) (((u8*)(buf))[(offset)] << 8 | ((u8*)(buf))[(offset) + 1])
#define read32(buf, offset) (((u8*)(buf))[(offset)] << 24 | ((u8*)(buf))[(offset) + 1] << 16 | ((u8*)(buf))[(offset) + 2] << 8 | ((u8*)(buf))[(offset) + 3])
...
{
	str->size = read32(top, 0);
	if (str->size == 0) {
		str->ptr = str->start = NULL;
	} else {
		str->ptr = str->start = top + 4;
	}
	str->bit = 0;
}
...
static s16 _readTree(Tree* const dst, BitBuffer* const str)
{
	...
	s16 no;
	s16 val;
	// STACK_PAD_VAR(2); (Pikmin fakematch; not in ED)
 ...
 * @TODO: Documentation
 */
static s16 decodeHuff(BitBufferWithTree* code)
{
	Tree* tree     = code->tree;
	BitBuffer* str = &code->str;
 ...
 */
	...
	block[0] = clipTable[c3 + v1 + t1 >> 3];
	block[1] = clipTable[c3 + v1 + t5 >> 3];
	block[2] = clipTable[c3 + v2 + t6 >> 3];
	block[3] = clipTable[c3 + v2 + t2 >> 3];
	block += blockWidth;
	...
	block[0] = clipTable[c3 + v1 - t5 >> 3];
	block[1] = clipTable[c3 - t3 >> 3];
	block[2] = clipTable[c3 - t4 >> 3];
	block[3] = clipTable[c3 + v2 - t6 >> 3];
	block += blockWidth;
	...
	block[0] = clipTable[c3 - v2 - t8 >> 3];
	block[1] = clipTable[c3 - t2 >> 3];
	block[2] = clipTable[c3 - t1 >> 3];
	block[3] = clipTable[c3 - v1 - t7 >> 3];
	block += blockWidth;

	block[0] = clipTable[c3 - v2 + t4 >> 3];
	block[1] = clipTable[c3 - v2 + t8 >> 3];
	block[2] = clipTable[c3 - v1 + t7 >> 3];
	block[3] = clipTable[c3 - v1 + t3 >> 3];
	block += blockWidth;
}

// 4x4 block of single value
static inline void dcBlock(u8* block, int blockWidth, u8 dc)
{
...
static s32 GetAotSum(VideoState* ws, long sum[16], u8 nbasis, u8* nestTop, int nestWidth, int p)
{
	long prev_scl;
	long scl;
	u8 bas[16];
...
static s32 GetMCAotSum(VideoState* ws, s32 sum[16], u8 nbasis, u8* nestTop, int nestWidth, int p)
{
	s32 prev_scl;
	s32 scl;
	u8 bas[16];
...
static void IntraAotBlock(VideoState* ws, u8* blk, int blkWidth, u8 dcv, u8 nbasis, int p)
{
	...
	if (nbasis == 6) {
		...
		avr = ((dcv + 0x80) << mns) - GetAotSum(ws, sum, nbasis, ws->nestBuf, ws->nestsize_h, p);

		blk[0] = clipTable[(sum[0] + avr) >> mns];
		blk[1] = clipTable[(sum[1] + avr) >> mns];
		blk[2] = clipTable[(sum[2] + avr) >> mns];
		blk[3] = clipTable[(sum[3] + avr) >> mns];
		blk += blkWidth;

		blk[0] = clipTable[(sum[4] + avr) >> mns];
		blk[1] = clipTable[(sum[5] + avr) >> mns];
		blk[2] = clipTable[(sum[6] + avr) >> mns];
		blk[3] = clipTable[(sum[7] + avr) >> mns];
		blk += blkWidth;

		blk[0] = clipTable[(sum[8] + avr) >> mns];
		blk[1] = clipTable[(sum[9] + avr) >> mns];
		blk[2] = clipTable[(sum[10] + avr) >> mns];
		blk[3] = clipTable[(sum[11] + avr) >> mns];
		blk += blkWidth;

		blk[0] = clipTable[(sum[12] + avr) >> mns];
		blk[1] = clipTable[(sum[13] + avr) >> mns];
		blk[2] = clipTable[(sum[14] + avr) >> mns];
		blk[3] = clipTable[(sum[15] + avr) >> mns];
		blk += blkWidth;
	}
}
...
static void _MotionComp(u8* cP, int cWidth, u8* tP, int tWidth, int dx, int dy)
{
	if (!dx && !dy) {
		_MotionComp_00(cP, cWidth, tP, tWidth);
		return;
	}
	if (dx && !dy) {
		_MotionComp_10(cP, cWidth, tP, tWidth);
		return;
	}
	if (!dx && dy) {
		_MotionComp_01(cP, cWidth, tP, tWidth);
		return;
	}
	if (dx && dy) {
		_MotionComp_11(cP, cWidth, tP, tWidth);
	}
}
...
static void MotionComp(VideoState* state, MCHandler* mch, int tx, int ty)
{
	...
	u8* tP;
	int offset;
	int plane_dx, plane_dy;

	for (c = 0; c < HVQM_PLANE_COUNT; c++) {
		pmc    = &mch->pln[c];
		sip    = &state->pln[c];
		plane_dx = (tx >> sip->h_shift + 1);
		plane_dy = (ty >> sip->v_shift + 1);
		t_top    = pmc->targ + (plane_dx + plane_dy * sip->plane_width);
		iofsP    = sip->imgUscan;
		...
		for (i = 0; i < blocks; i++) {
			...
			tP     = t_top + offset;
			_MotionComp(cP, sip->plane_width, tP, sip->plane_width, tx & 1, ty & 1);
		}
	}
}
 ...
 * @TODO: Documentation
 */
static void PrediAotBlock(VideoState* ws, u8* blk, u8* mblk, int blkWidth, u8 nbasis, u8* nestPtr, int nestWidth, int p, int dx, int dy)
{
	s32 sum[16];
	u8 mvc[16];
	int mov[16];
	...
	_MotionComp(mvc, 4, mblk, blkWidth, dx, dy);
		...
		for (i = 0; i < blocks; i++) {
			...
			if (nbasis == 6) {
				...
				if (nbasis == 0) {
					...
					PrediAotBlock(ws, cP, tP, sip->plane_width, nbasis, nestP, ws->pln[Y_IDX].plane_width, c, tx & 1, ty & 1);
				}
			}
		}
	}
...
static int getMCBtype(BitBufferWithTree* code, RLDecoder* flag)
{
	...
	if (flag->runlng == 0) {
		...
		flag->status = mcbtypetrans[getBit(&code->str) ? 1 : 0][flag->status];
		flag->runlng = decodeUOvfSym(code, 0xFF);
	}
	flag->runlng--;
	return flag->status;
...
{
	// HVQM4 1.3 reads the big-endian sizes a byte at a time
	obj->frame_width  = ((u8*)&header->width)[0] << 8 | ((u8*)&header->width)[1];
	obj->frame_height = ((u8*)&header->height)[0] << 8 | ((u8*)&header->height)[1];
