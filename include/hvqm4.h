...
typedef struct Tree {
	...
	s16 leaf[2][512]; // _08, binary tree nodes [0/1 branch][node index] (HVQM4 1.3: s16)
} Tree;
...
typedef struct BitBuffer {
	...
	u8* start; // _04, start of the stream (HVQM4 1.3 only)
	u32 size;  // _08, remaining data size
	u8 value;  // _0C, byte being read
	u8 bit;    // _0D, mask of the next bit in value, 0 when a new byte is needed
} BitBuffer;
...
typedef struct VideoState {
	...
	Tree zrn_tree;                             // _08B0, zero run-length tree
	Tree scl_tree;                             // _10B8, scale factor tree
	Tree num_tree;                             // _18C0, basis number tree
	Tree vec_tree;                             // _20C8, motion vector tree
	Tree mrn_tree;                             // _28D0, macroblock run tree
	BitBufferWithTree dcval[HVQM_PLANE_COUNT]; // _30D8, DC coefficient streams
	BitBufferWithTree dcrun[HVQM_PLANE_COUNT]; // _3114, DC run-length streams
	BitBufferWithTree scale[HVQM_PLANE_COUNT]; // _3150, AOT scale streams
	BitBufferWithTree bsnum[HVQM_LUMA_CHROMA]; // _318C, basis function indices
	BitBufferWithTree bsrun[HVQM_LUMA_CHROMA]; // _31B4, basis run-lengths
	BitBuffer aotcd[HVQM_PLANE_COUNT];         // _31DC, AOT coefficient data
	BitBufferWithTree mvecx;                   // _320C, horizontal motion vectors
	BitBufferWithTree mvecy;                   // _3220, vertical motion vectors
	BitBufferWithTree mcaot;                   // _3234, macroblock processing flags
	BitBufferWithTree mstat;                   // _3248, macroblock type flags
	u16 nestsize_h;                            // _325C, reference nest width
	u16 nestsize_v;                            // _325E, reference nest height
	u8 landscape;                              // _3260, orientation flag
	u8 nestBuf[70 * 38];                       // _3261, downsampled reference
	s16 dc_max;                                // _3CC6, DC coefficient max (HVQM4 1.3: s16)
	s16 dc_min;                                // _3CC8, DC coefficient min
	u8 aotscale_q;                             // _3CCA, AOT quantization shift
	u8 dc_scale_q;                             // _3CCB, DC quantization shift
	u8 fcode[2][2];                            // _3CCC, motion vector precision [axis:0=x,1=y][ref:0=past,1=future]
} VideoState;
